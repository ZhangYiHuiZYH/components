...
export default {
  ...
  methods: {
    ...
    childCheckedHandle(node, checked, halfcheck = false) {
      this.$set(node, "selected", checked); // 只对当前的selected属性有效
      const { children } = node;
      if (children && children.length) {
        children.forEach(child => {
          this.$set(child, "checked", false);
          this.$set(child, "selected", false);
          this.childCheckedHandle(child, checked=false, halfcheck = false);
        });
      }
    },
    parentCheckedHandle(parentNode, checked, halfcheck = false) {
      ...
      this.parentCheckedHandle(parentNode, checked=true, halfcheck = false);
      // this.$set(parentNode, "checked", true);
      // this.$set(parentNode, "selected", true);
      ...
      //   this.$set(parentNode, "selected", parentChecked);
      // } else {
      //   this.$set(parentNode, "checked", allBortherNodeChecked);        
      //   this.$set(parentNode, "selected", allBortherNodeChecked); 
      // }
      //return true;
    },
    ...
    nodeSelected(node, position) {
      ...
      this.childCheckedHandle(node, selected, false);
      this.parentCheckedHandle(node, selected, true);
